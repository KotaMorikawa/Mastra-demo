...
export const fileProcessorTool = createTool({
  ...
  inputSchema: z.object({
    filePath: z.string().describe("処理するファイルパス"),
    strategy: z
      .enum([
        "recursive",
        "character",
      ...
      .default("recursive")
      .describe("チャンキング戦略"),
    chunkSize: z.number().default(512).describe("チャンクサイズ"),
    ...
    dbPath: z
      ...
      .describe("ベクトルストアのパス"),
    indexName: z.string().describe("ベクトルインデックス名"),
  }),
  ...
  execute: async ({ context }) => {
    ...
    try {
      ...
      console.log(
        `ファイル ${filePath} のチャンク処理成功: ${chunks.length} 件`
      );

      try {
        // LibSQLVectorのライブラリを使用してベクトルストアを初期化
        ...
        // 埋め込みベクトルを生成
        // Googleのモデルは一度に最大100個のリクエストしか処理できないため、バッチ処理を実装する
        const batchSize = 100;
        let allEmbeddings = [];
        ...
        return {
          success: true,
          message: `ファイル ${filePath} のチャンキングと埋め込みが完了しました。 ${chunks.length} 件のチャンクをベクトルストアに保存しました。`,
          filePath,
          strategy,
          fileType,
        ...
        return {
          success: false,
          message: `ファイル ${filePath} のチャンキングは成功しましたが、埋め込み処理に失敗しました。`,
          filePath,
          strategy,
          chunkCount: chunks.length,
          fileType,
